    ...
    <ContentLayout>
      <div className="mx-auto max-w-2xl py-6 space-y-8 animate-in fade-in-50 duration-500">
        ...
        <div className="space-y-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight text-foreground">Create Monitor</h1>
            <p className="text-sm text-muted-foreground mt-1">Configure your monitor settings and endpoints.</p>
          </div>
        </div>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <Select onValueChange={(val) => field.onChange(Number(val))} defaultValue={field.value?.toString()}>
                        ...
                        <SelectContent>
                          {plan === 'PRO_PLUS' && <SelectItem value="30" className="text-xs">Every 30 seconds</SelectItem>}
                          {(plan === 'PRO' || plan === 'PRO_PLUS') && <SelectItem value="60" className="text-xs">Every 1 minute</SelectItem>}
                          <SelectItem value="300" className="text-xs" disabled={plan === 'FREE'}>Every 5 minutes</SelectItem>
                          <SelectItem value="600" className="text-xs" disabled={plan === 'FREE'}>Every 10 minutes</SelectItem>
                      ...
                      {plan === 'FREE' && (
                        <p className="text-[10px] text-primary mt-1.5 flex items-center gap-1 font-medium">
                          <Sparkles className="h-3 w-3" /> Upgrade to Pro for 1-minute checks
                        </p>
                      )}
                      <FormMessage />
                    </FormItem>
            ...
            {/* Submit */}
            <div className="pt-4">
              <Button type="submit" disabled={!isFormReady} className="w-full rounded-md h-11 font-bold">
                Create Monitor
              </Button>
            </div>
          </form>
